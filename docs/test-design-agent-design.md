...
- 对测试点做轻量规范化，保留测试点 ID、模块、类型、方法、等级、需求依据和风险备注等来源信息。
- 将测试点扩展为完整测试用例，至少包含用例名称、用例编号、用例等级、前置步骤、测试步骤和预期结果。
- 保证每条测试用例可独立执行、步骤清晰、预期结果可判定。
- 基于测试点类型、风险等级和测试方法选择合适的用例设计策略，包括正向路径、异常路径、边界数据、状态流转、权限角色、接口契约、数据一致性和组合兼容。
...
Test-Design-Agent 的主输入是测试点，推荐来自 Test-Analysis-Agent 的测试点明细文件：
...
| `templates/` | 最终报告、独立用例明细和澄清会话产物格式 |
