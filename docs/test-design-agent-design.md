...
Test-Design-Agent 参考 Test-Analysis-Agent 的分层设计：使用 Skill 承载主流程和设计动作，使用 Knowledge 维护稳定测试设计标准，使用 Memory 注入项目上下文和历史经验，使用 Template 约束输出格式，使用 Quality Gate 和 lint 脚本形成确定性质量闭环。
...
- 同时兼容 Claude Code plugin 和 OpenCode 项目运行方式，保持同一套 skill 事实源。
...
系统对外暴露一个稳定能力：基于测试点生成规范测试用例。主流程由入口 skill 固化，避免多个入口维护重复流程。
...
运行时路径解析规则：文档和 Skill 中出现的 `skills/...`、`knowledge/...`、`templates/...`、`quality-gates/...`、`memory/...`、`bin/...` 和 `outputs/...` 均为仓库根目录相对路径，不是当前 skill 或输入文件目录的相对路径。执行 `testcase-design` 时不得查找 `skills/testcase-design/knowledge/`。
