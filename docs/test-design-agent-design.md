...
- 基于测试点类型、风险等级和场景级设计约束推导合适的测试设计方法与用例设计策略，包括正向路径、异常路径、边界数据、状态流转、权限角色、接口契约、数据一致性和组合兼容。
...
Test-Design-Agent 的推荐输入是测试用例设计输入包，结构为“需求 -> 测试场景 -> 设计约束 -> 测试点”。其中设计约束绑定在测试场景上，供同一场景下的多个测试点复用。
...
| 用例等级 | 是 | `Level 0` 至 `Level 4`，来源于测试点风险和业务影响 |
| 前置步骤 | 是 | 执行前必须满足的环境、数据、账号、状态准备 |
| 测试步骤 | 是 | 可顺序执行的操作步骤，编号清晰 |
| 预期结果 | 是 | 与测试步骤逐条或分组对应，必须可观察、可判定 |
...
| 追溯完整 | 通过追溯矩阵关联测试点与测试用例，每个有效测试点至少被一条用例覆盖 |
...
| 路径 | 内容范围 | 输出场景 |
|---|---|---|
| `templates/testcase-design-input-template.md` | 测试用例设计输入包，包含需求信息、测试场景清单、场景级设计约束、测试点、待确认信息和输入完整性自检；测试场景清单定义业务情境和范围，设计约束定义执行条件、判定规则和观察方式 | 输入准备、评审前补充上下文 |
...
| 测试点等级 | 默认扩展策略 |
|---|---|
| Level 0 | 至少覆盖主路径、关键异常、关键边界、数据一致性或权限风险 |
| Level 1 | 覆盖主路径和主要异常，必要时补充边界或状态用例 |
| Level 2 | 覆盖代表性正向或异常场景 |
| Level 3 | 覆盖轻量检查，避免生成冗余用例 |
| Level 4 | 默认作为候选或补充用例，除非 memory 或用户要求纳入 |
