...
Test-Design-Agent 的推荐输入是测试用例设计输入包，结构为“需求 -> 测试场景 -> 设计约束 -> 测试点”，并将接口测试对象拆分为“接口测试清单 -> 接口测试详情 -> 接口测试点”。其中设计约束绑定在测试场景上，接口设计约束绑定在接口测试对象上。
...
| 路径 | 内容范围 | 输出场景 |
|---|---|---|
| `templates/testcase-design-input-template.md` | 测试用例设计输入包，包含需求信息、测试场景清单、测试场景详情、接口测试清单、接口测试详情、待确认信息和输入完整性自检；测试场景承载页面、业务流程、资料、性能、可靠性等对象，接口测试对象承载接口契约、字段、错误码、幂等和鉴权等对象 | 输入准备、评审前补充上下文 |
