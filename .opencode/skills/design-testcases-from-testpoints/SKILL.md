...
description: 主入口 skill。当用户提供测试用例设计输入包、测试点文件或测试点文本，并要求生成测试用例设计报告、可执行测试用例明细、追溯矩阵、覆盖审查或质量门禁结果时使用；负责从 $ARGUMENTS 串联 memory 上下文、测试点规范化、待确认治理、测试设计、用例编写、审查校验和 Markdown 产物输出。
---
...
`run-id` 格式为 `<YYYYMMDD-HHMMSS>-<测试点文件名安全短名>-<短哈希>`。同一轮设计内的待确认记录、修正和质量门禁重跑必须复用同一个运行目录。

## 无人值守执行策略

- 默认连续执行到完整测试用例设计报告和独立测试用例明细落盘，不在阶段之间等待用户输入。
- 运行过程中不得中断流程向用户提问；所有信息缺口、事实冲突和确认建议都统一写入最终报告的“待确认问题”。
- `MustConfirm`、`ShouldConfirm` 和 `RecordOnly` 问题均写入待确认问题，并继续生成保守可执行用例；确实无法可靠生成的测试点在追溯矩阵和质量门禁中标记为 `待确认`。
...
2. 创建运行目录，写出 `context-pack.md`、`clarification-session.md` 和 `testcase-design-plan.md` 的初始文件；可使用 `bin/run-test-design.py <输入文件>` 准备这些运行产物。该脚本只准备运行目录和校验已有产物，不负责调用模型生成测试用例。
3. 使用 `memory-context-builder` 或 `bin/build-memory-context.py` 构建本次相关上下文；必须让其枚举并匹配 `memory/domains/*.md` 中的本地业务域记忆，context-pack 中应写明命中的 domain 文件名和匹配依据，未命中时也要明确说明。
...
5. 在 `CP-INPUT` 使用 `clarification-gate` 收集、分级和去重待确认问题，不向用户提问，也不阻塞后续阶段。
6. 使用 `testcase-design` 为每条测试点选择设计模式、覆盖意图、用例标题建议、前置关注、步骤关注、预期关注和等级，设计方案结构遵循 `templates/testcase-design-plan-template.md`。
7. 使用 `testcase-writing` 生成测试用例明细。单条用例只包含用例编号、用例名称、用例等级、前置步骤、测试步骤和预期结果。
8. 对独立测试用例明细先运行仓库根目录下的 `bin/lint-testcase-report.py` 和 `bin/semantic-testcase-check.py`；如失败，将确定性错误交回 `testcase-writing` 定向修正，并在同一运行目录重跑脚本，直到通过或记录阻塞原因。lint 会拦截 `<...>` 和 `{{...}}` 等未替换模板占位符。
...
10. 对完整报告再次运行 `bin/lint-testcase-report.py <报告路径> --source <输入文件>` 和 `bin/semantic-testcase-check.py`；如失败，优先修正报告结构、追溯引用、源测试点覆盖或脚本明确指出的单点问题，不重新展开全量评审。
11. 输出：
...
- 报告必须包含测试点输入摘要、待确认处理摘要、用例设计策略、测试用例明细、追溯矩阵、覆盖审查、质量门禁、专家评分、待确认问题和 memory 更新建议。
