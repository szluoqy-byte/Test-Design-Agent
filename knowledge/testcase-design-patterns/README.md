...
4. 如果测试点只有“验证功能正常”之类泛化描述，应先进入澄清或降级为场景测试，不直接扩展大量用例。
...
| 测试点等级 | 默认策略 |
|---|---|
| Level 0 | 覆盖主路径、关键异常、必要边界或防护场景 |
| Level 1 | 覆盖主路径和主要异常，必要时补充边界 |
| Level 2 | 覆盖代表性正向或异常场景 |
| Level 3 | 轻量覆盖，避免重复和穷举 |
| Level 4 | 默认作为候选补充，除非用户或 memory 要求纳入 |
