...
可使用仓库根目录下的 `bin/build-memory-context.py` 机械生成 context-pack 初稿，再由本 skill 进行语义补充和冲突判断。
...
在生成 `context-pack.md` 前，必须先枚举 `memory/domains/*.md` 的文件名、metadata、一级标题和可识别入口路径，再按以下信号选择相关 domain：

- 若 domain 文件头声明了 `keywords`、`aliases` 或 `entry_paths`，优先使用这些稳定 metadata 匹配。
...
- 只有当前输入和 domain memory 在同一入口层级给出不同菜单、页面、页签名称，或当前输入明确声明另一条完整路径时，才视为冲突。冲突时当前输入优先，并在“可能冲突或需要用户确认的信息”中记录冲突，不直接用 domain memory 覆盖。`bin/build-memory-context.py` 可机械识别同一目标入口路径不一致的冲突并写入 context-pack。
