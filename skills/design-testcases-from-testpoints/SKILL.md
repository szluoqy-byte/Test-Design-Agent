...
本 skill 从 `$ARGUMENTS` 指定的测试用例设计输入包、测试点文件或用户粘贴的测试点文本生成测试用例设计报告和独立测试用例明细。推荐输入包模板见 `templates/testcase-design-input-template.md`。
...
4. 规范化输入：优先识别“需求 -> 测试场景 -> 设计约束 -> 测试点”结构；若输入只有测试点，则识别 `TP-*`、模块、测试点、类型、需求依据、级别、风险备注，保留原始编号。测试设计方法和预期判定由后续设计阶段推导，不要求输入测试点携带。
